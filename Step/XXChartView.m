...
- (void)dravLine{
    ...
    [color set];
    [path moveToPoint:CGPointMake( label.frame.origin.x - bounceX, (6000 -arc4random()%6000) /6000.0 * (self.frame.size.height - bounceY*2 )  )];
    
    //创建折现点标记
    for (NSInteger i = 1; i< 12; i++) {
        UILabel * label1 = (UILabel*)[self viewWithTag:1000 + i];
        CGFloat  arc = arc4random()%6000;
        [path addLineToPoint:CGPointMake(label1.frame.origin.x - bounceX,  (6000 -arc) /6000.0 * (self.frame.size.height - bounceY*2 ) )];
        UILabel * falglabel = [[UILabel alloc]initWithFrame:CGRectMake(label1.frame.origin.x , (6000 -arc) /6000.0 * (self.frame.size.height - bounceY*2 )+ bounceY  , 30, 15)];
...
- (void)dravLine:(NSArray *)dataArray{
    ...
    if (dataArray.count > 0) {
        [path moveToPoint:CGPointMake( label.frame.origin.x - bounceX, (6000 - [dataArray[0] floatValue] ) /6000.0 * (self.frame.size.height - bounceY*2 )  )];
    }
    ...
    for (NSInteger i = 1; i< dataArray.count; i++) {
        UILabel * label1 = (UILabel*)[self viewWithTag:1000 + i];
        //        CGFloat  arc = arc4random()%6000;
        CGFloat arc = [dataArray[i] floatValue];
...
#pragma mark 创建x轴的数据
- (void)createLabelX:(NSArray *)titleArray{
    CGFloat  month = titleArray.count;
    for (NSInteger i = 0; i < month; i++) {
        UILabel * LabelMonth = [[UILabel alloc]initWithFrame:CGRectMake((self.frame.size.width - 2*bounceX)/month * i + bounceX, self.frame.size.height - bounceY + bounceY*0.3, (self.frame.size.width - 2*bounceX)/month-4, bounceY/2)];
        //       LabelMonth.backgroundColor = [UIColor greenColor];
        LabelMonth.tag = 1000 + i;
        LabelMonth.text = [NSString stringWithFormat:@"%@",titleArray[i]];
...
- (void)createLabelY{
    ...
    for (NSInteger i = 0; i < Ydivision; i++) {
        ...
        labelYdivision.text = [NSString stringWithFormat:@"%.0f",(Ydivision - i)*1000];
